...
def RM_EG(X_training, y_training, sigman = 1.0, m_fun = None, Dm_fun = None, k_fun = None, Dk_fun = None, DDk_fun = None,
    ...
    def jacobian_mom(X_test):
        ...
        return mu_post, cov_post+jnp.eye(dim)*delta_stable #SOMETHING WRONG COV IS NOT POSTIVE (SYMMETRIC DEFINITE), BUT IS NEGATIVE SYMMETRIC DEFINITE
    ...
    def Emmf(X_test):
        ...
        EG = mu_post.T.dot(mu_post)+N_obs*cov_post
    ...
    def DEJ(X_test):
        ...
        DDm_test = DDm_fun(X_test)
        
        if N_obs == 1:
            return DDm_test+solved1.dot(y_training-m_training)-solved2.dot(DK11.dot(K11_inv).dot(y_training-m_training))-solved2.dot(Dm_training)
        else:
            return vmap(lambda y: DDm_test+solved1.dot(y-m_training)-solved2.dot(DK11.dot(K11_inv).dot(y-m_training))-solved2.dot(Dm_training))(y_training)
    ...
    if Dm_fun is None and m_fun is not None:
        ...
        #DDm_fun = lambda x: jnp.zeros((x.shape[-1], x.shape[-1]))
        DDm_fun = lambda x: jnp.zeros(x.shape[-1])
...
def RM_SG(X_training, y_training, sigman = 1.0, m_fun = None, Dm_fun = None, k_fun = None, Dk_fun = None, 
    ...
    def ivp_geodesic(x,v, eps):
        ...
        def eq_geodesic(t, y, epsilon):
                        
            gamma = y[0:N]
            Dgamma = y[N:]
            
            chris = RM.chris(gamma, epsilon)
        ...
        def step_fun(yn, time):
            ...
            tn, hn, epsilon = time
            y = yn+eq_geodesic(tn,yn, epsilon)*hn
        ...
        step = len(dt_grid)
        y0 = jnp.concatenate((x, v), axis=0)
        _, y = lax.scan(step_fun, y0, xs=(grid[1:], dt_grid, eps))
        y = jnp.concatenate((y0[jnp.newaxis,...],y), axis=0)
    ...
    def DDmmf(X_test, epsilon):
        ...
        DL = 0.5*jnp.einsum('ijk, lj->ilk', DSigma, L_inv)
        DDL = 0.5*jnp.einsum('idk,ij->jdk', DSigma, L.dot(L.T))
        DDL = jnp.einsum('jdk,jim->imdk', DDL, DSigma)
    ...
    RM.chris = chris_symbols
    RM.Dchris = Dchris
    
    RM.CO = curvature_operator
    RM.CT = curvature_tensor
    RM.SC = sectional_curvature
