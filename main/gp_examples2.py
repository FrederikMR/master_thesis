...
import gaussian_process2 as gp
import sp
...
eps = sp.sim_multinormal(mu=jnp.zeros(2), cov=jnp.eye(2), dim=N_sim*99*3).reshape(N_sim, 99, 2, 3)
...
eps = sp.sim_multinormal(mu=jnp.zeros(2), cov=jnp.eye(2), dim=N_sim*3*n_points*n_points).reshape(N_sim, n_points, n_points, 2, 3)
for i in range(N_sim):
    sec = vmap(lambda y1, eps1: vmap(lambda y2, eps2: RMSG.SC(y2, jnp.array([1.0, 0.0]), jnp.array([0.0, 1.0]), eps2))(y1, eps1))(X, eps[i])
    #X1_plot
    #X2_plot
    #sec_plot = sec[sec<10]
