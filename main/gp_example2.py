...
eps = sp.sim_multinormal(mu=jnp.zeros(2), cov=jnp.eye(2), dim=N_sim*n_points*n_points*3).reshape(N_sim, n_points, n_points, 2, 3)
...
for i in range(N_sim):
    sec = vmap(lambda y1, e1: vmap(lambda y2, e2: RMSG.SC(y2, jnp.array([1.0, 0.0]), jnp.array([0.0, 1.0]), e2))(y1, e1))(X, eps[i])
    #X1_plot
    #X2_plot
    #sec_plot = sec[sec<10]
