...
eps = sp.sim_multinormal(mu=jnp.zeros(2), cov=jnp.eye(2), dim=N_sim*28*28).reshape(N_sim, 2, 28*28)
...
img_base = jnp.array(train_data.data[0])
...
rot_plot = 1.0*np.asarray(rot.reshape(num_rotate, 1, 28, 28))
rot_plot = torch.from_numpy(rot_plot)
plt_idx = 2*np.arange(0,99)
rot_plot2 = rot_plot[plt_idx]
rot_plot = torch.cat((rot_plot2, rot_plot[-1].reshape(-1,1,28,28)))
...
#Loading model
model = VAE_MNIST().to(device) #Model used
optimizer = optim.Adam(model.parameters(), lr=lr)

checkpoint = torch.load('trained_models/mnist/mnist_epoch_50000.pt', map_location=device)
...
rec_data = rec_data.T.reshape(-1,28,28,1)
...
real_batch = torch.Tensor(np.array(rot.reshape(-1,28*28)))
recon_batch = model(real_batch) #x=z, x_hat, mu, var, kld.mean(), rec_loss.mean(), elbo
x_hat = recon_batch[1].detach().reshape(-1,1,28,28)

plt_idx = 2*np.arange(0,99)
rot_plot2 = x_hat[plt_idx]
rot_plot = torch.cat((rot_plot2, x_hat[-1].reshape(-1,1,28,28)))

# Plot some training images
plt.figure(figsize=(8,6))
plt.axis("off")
plt.title("Reconstruction Rotated Images for VAE")
plt.imshow(np.transpose(vutils.make_grid(rot_plot.to(device), padding=2, normalize=True, nrow=10).cpu(),(1,2,0)))
plt.show()
...
gammaEG_manifold = gammaEG_manifold.T.reshape(-1,28,28, 1)
...
load_path = 'rm_computations/mnist_geodesic.pt'
...
G_plot = checkpoint['G_plot'].reshape(-1,1,28,28)
...
rot_plot = torch.cat((rot_plot2, rot_plot10[-1].reshape(-1,1,28,28)))
ax[0].imshow(vutils.make_grid(rot_plot, padding=2, normalize=True, nrow=10).permute(1, 2, 0))
ax[0].axes.get_xaxis().set_visible(False)
ax[0].set_yticks([28/2])
...
EG_error = jnp.linalg.norm((gamma_true-gammaEG_manifold).reshape(100,28*28), axis=1)
VAE_error = jnp.linalg.norm((gamma_true-jnp.einsum('ijkm->ikmj', jnp.array(G_plot.detach().numpy()))).reshape(100,28*28), axis=1)
SG_error = []
for i in range(N_sim):
    SG_error.append(jnp.linalg.norm((gammaSG_manifold[i].T.reshape(-1,28,28,1)-gamma_true).reshape(100,28*28), axis=1))
