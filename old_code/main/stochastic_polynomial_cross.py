...
param_fun = sym.Matrix([x1, x2, a*(x1**2)+b*(x2**2)+c*x1*x2])
...
for i in range(np.min((N_sim,10))):
    Z = coef[i][1]*(X**2)+coef[i][1]*(Y**2)+coef[i][2]*X*Y
...
EG = sym.Matrix([[4*x1**2+x2**2+1,x1*x2], [x1*x2,4*x2**2+x1**2+1]])
...
for i in range(len(center)):
    ...
    for j in range(N_sim):
        G = SG.subs(a,coef[j][0]).subs(b,coef[j][1]).subs(c,coef[j][2])
...
sec_coef = sym.lambdify(x, sec, modules='numpy')
if sec == 0.0:
...
for i in range(np.min((N_sim,10))):
    sec_coef = sec.subs(a,coef[j][0]).subs(b,coef[j][1]).subs(c,coef[j][2])
...
for i in range(N_sim):
    G = SG.subs(a,coef[i][0]).subs(b,coef[i][1]).subs(c,coef[i][2])
...
K_SG = K_SG.subs(x1, 0).subs(x2,0)
K_EG = float(K_EG.subs(x1, 0).subs(x2,0))

N_sim = 10000
coef = np.random.normal(mu, sigma, [N_sim, 3])
K_SG_list = []
for i in range(N_sim):
    K_SG_list.append(float(K_SG.subs(a, coef[i,0]).subs(b,coef[i,1]).subs(c,coef[i,2])))
...
#MEAN = -1, VAR=18, DISTRIBUTION: SIMILAR TO SIMPLE CASE, BUT WITH EXTRA CHI_SQUARED DISTRIBUTION
