...
param_fun = sym.Matrix([x1, x2, a*(x1**2)+b*(x2**2)])
...
EG = sym.Matrix([[4*x1**2+1,0], [0,4*x2**2+1]])
...
K_SG_list = []
for i in range(N_sim):
    K_SG_list.append(float(K_SG.subs(a, coef[i,0]).subs(b,coef[i,1])))
